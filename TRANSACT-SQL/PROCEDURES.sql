...
BEGIN 
	...
	BEGIN TRY
		BEGIN TRANSACTION 
			...
			(SELECT CASE WHEN @intIdPadre IS NULL THEN 
							(SELECT CASE WHEN MAX(POSICION) IS NULL THEN 1 
									ELSE MAX(POSICION)+1 END FROM TBLMENU WHERE ID = IDPADRE) 
						ELSE (SELECT MAX(POSICION)+1 FROM TBLMENU WHERE IDPADRE = @intIdPadre) END FROM TBLMENU), 
			(SELECT CASE WHEN @intIdPadre IS NULL THEN 
						(SELECT CASE WHEN MAX(ID) IS NULL THEN 1 
					ELSE MAX(ID)+1 END FROM TBLMENU) ELSE @intIdPadre END FROM TBLMENU))
			COMMIT TRANSACTION;
	 END TRY
	 BEGIN CATCH
		IF @@TRANCOUNT > 0
